...
TEST(SharedTypesLayout, PreprocessParamsRoundTrips) {
  glint::gpu::Device device;

  PreprocessParams params{};
  params.view_matrix = simd::float4x4(
      simd::float4{1.0f, 2.0f, 3.0f, 4.0f}, simd::float4{5.0f, 6.0f, 7.0f, 8.0f},
      simd::float4{9.0f, 10.0f, 11.0f, 12.0f}, simd::float4{13.0f, 14.0f, 15.0f, 16.0f});
  params.camera_position_x = 0.5f;
  params.camera_position_y = -1.5f;
  params.camera_position_z = 2.5f;
  params.fx = 600.0f;
  params.fy = 601.0f;
  params.cx = 599.5f;
  params.cy = 339.5f;
  params.image_width = 1200;
  params.image_height = 680;

  glint::gpu::Buffer<PreprocessParams> params_buffer(device.device(), 1);
  params_buffer.data()[0] = params;

  glint::gpu::Buffer<float> out_buffer(device.device(), 25);

  glint::gpu::Kernel kernel(device.device(), device.library(), "read_preprocess_params");
  glint::gpu::dispatch_and_wait(device.queue(), kernel,
                                 {params_buffer.handle(), out_buffer.handle()}, 1);
  ...
  for (int col = 0; col < 4; ++col) {
    for (int row = 0; row < 4; ++row) {
      EXPECT_FLOAT_EQ(out[col * 4 + row], params.view_matrix.columns[col][row]);
    }
  }
  EXPECT_FLOAT_EQ(out[16], params.camera_position_x);
  EXPECT_FLOAT_EQ(out[17], params.camera_position_y);
  EXPECT_FLOAT_EQ(out[18], params.camera_position_z);
  EXPECT_FLOAT_EQ(out[19], params.fx);
  EXPECT_FLOAT_EQ(out[20], params.fy);
  EXPECT_FLOAT_EQ(out[21], params.cx);
  EXPECT_FLOAT_EQ(out[22], params.cy);
  EXPECT_FLOAT_EQ(out[23], static_cast<float>(params.image_width));
  EXPECT_FLOAT_EQ(out[24], static_cast<float>(params.image_height));
}
